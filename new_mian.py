...
- HuggingFace Trainer 做反向与日志；默认每 logging_steps 在 stdout 打一行可读指标（loss/lr/grad_norm/epoch），eval 打 eval_loss；可加 --tqdm 显示进度条。
- nohup 到文件时建议: PYTHONUNBUFFERED=1 python3 -u new_mian.py ...
...
class ReadableMetricsCallback(TrainerCallback):
    """把关键标量打成单行，便于 nohup 日志里 grep / 调参。"""
    ...
    def on_log(self, args, state, control, logs: Optional[Dict[str, float]] = None, **kwargs):
        ...
        if "epoch" in logs and logs["epoch"] is not None:
            parts.append(f"epoch={float(logs['epoch']):.4f}")
        if parts:
            print(f"[train] step={state.global_step} " + " ".join(parts), flush=True)
    ...
    def on_evaluate(self, args, state, control, metrics=None, **kwargs):
        ...
        if "eval_loss" in metrics:
            print(
                f"[eval]  step={state.global_step} eval_loss={float(metrics['eval_loss']):.6f}",
                flush=True,
            )
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=int,
        default=100,
        help="每隔多少 optimizer step 做一次验证（调参冒烟时可改小，如 20）",
    )
    parser.add_argument(
        "--save_steps",
        type=int,
    ...
    parser.add_argument(
        "--tqdm",
        action="store_true",
        help="显示 Trainer 进度条（默认关闭，避免 \\r 与单行指标混在一起）",
    )
    parser.add_argument(
        "--no_readable_metrics",
        action="store_true",
...
def main():
    ...
    print(
        "  - 超参摘要: "
        f"lr={args.lr} per_device_batch={args.batch_size} grad_accum={args.grad_accum} "
    ...
    _log_extra: Dict[str, Any] = {"disable_tqdm": not args.tqdm}
    ...
    callbacks = []
    if not args.no_readable_metrics:
        callbacks.append(ReadableMetricsCallback())
    ...
    print(f"训练完成，模型已保存到: {args.output_dir}", flush=True)
    if getattr(trainer.state, "best_metric", None) is not None:
        print(f"  - best_metric ({training_args.metric_for_best_model}): {trainer.state.best_metric}", flush=True)
    hist = trainer.state.log_history
    if hist:
        tail = hist[-12:]
        print("  - log_history 末尾（便于对照曲线）:", flush=True)
        print(json.dumps(tail, ensure_ascii=False, indent=2), flush=True)
