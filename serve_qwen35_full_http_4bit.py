...
def build_parser() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Qwen3.5-4B full-model HTTP server (8bit)")
...
def create_state(args: argparse.Namespace) -> Dict[str, Any]:
    ...
    # P100 在实际生成阶段经常触发 int8 cublasLt 错误，直接使用 4bit 更稳定。
    prefer_4bit = "P100" in gpu_name.upper()
    if prefer_4bit:
        print(f"[warn] detected gpu={gpu_name}, use 4bit for stability")
        mode = "4bit_fallback"
...
def make_handler(state: Dict[str, Any]):
    ...
    class Handler(BaseHTTPRequestHandler):
        ...
        def do_GET(self):  # noqa: N802
            if self.path == "/health":
                self._resp(200, {"ok": True, "mode": mode})
                return
